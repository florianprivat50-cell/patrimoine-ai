...
import { login, signup, requestPasswordRecovery, updateUser, acceptInvite, getSettings, type Settings } from '@netlify/identity';
...
export function AccountBoundary({children}:{children:ReactNode}) {
 ...
 if(!ready&&status==='error')return <div className="account-shell"><p role="alert">{message}</p><AccountPage forceLogin/><button className="btn-ghost" onClick={()=>void retryAccount()}>Réessayer le chargement</button></div>;
 if(!ready)return <div className="account-shell"><section className="account-card"><span className="eyebrow">PATRIMOINE AI</span><h1>Ouverture de votre espace…</h1><p role="status">{message||'Chargement de vos données personnelles.'}</p></section></div>;
 return <div key={`${user?.id??'guest'}:${generation}`}>{children}</div>;
}
export function AccountStatus(){
 const {user,status,updatedAt,message,authMessage}=useAccount();
 ...
 return <div className={`account-status account-status-${status}`}><Link to="/compte">{label}</Link>{user&&<span>{user.email}</span>}{status==='saved'&&updatedAt&&<small>{new Date(updatedAt).toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'})}</small>}{user&&['error','pending'].includes(status)&&<button onClick={()=>void retryAccount()}>Réessayer</button>}{message&&<p role="status">{message}</p>}{authMessage&&<p role="status">{authMessage}<button onClick={()=>useAccount.setState({authMessage:''})}>Fermer le message</button></p>}</div>;
}
export default function AccountPage({forceLogin=false}:{forceLogin?:boolean}){
 const account=useAccount();const [reauth,setReauth]=useState(forceLogin);
 ...
 const [settings,setSettings]=useState<Settings|null>(null),[settingsError,setSettingsError]=useState(false);
 useEffect(()=>{let active=true;void getSettings().then(value=>{if(active)setSettings(value);}).catch(()=>{if(active)setSettingsError(true);});return()=>{active=false;};},[]);
 const signupAllowed=!!settings&&!settings.disableSignup&&!settings.autoconfirm;
 const reset=account.resetPassword||!!account.inviteToken;
 async function action(fn:()=>Promise<unknown>|unknown){setBusy(true);setError('');setNotice('');try{await fn();}catch(e:any){setError(e.status===429?'Trop de tentatives. Patientez quelques minutes avant de réessayer.':e.status===401||e.status===400?'Adresse e-mail ou mot de passe incorrect, adresse non confirmée ou lien expiré.':e.status===422?'Vérifiez votre adresse e-mail et choisissez un mot de passe plus robuste.':e.status===403?'Cette inscription n’est pas autorisée. Vérifiez votre e-mail ou contactez le gestionnaire du site.':e.message||'Service indisponible. Réessayez.');}finally{setBusy(false);setPassword('');}}
 function submit(e:FormEvent){e.preventDefault();void action(async()=>{
  ...
  else if(mode==='signup'){if(!signupAllowed)throw new Error('L’inscription avec confirmation e-mail n’est pas disponible pour le moment.');await signup(email.trim(),password,{full_name:name.trim()});setMode('login');setNotice('Consultez votre boîte e-mail, y compris les indésirables, pour confirmer votre inscription avant de vous connecter.');}
 ...
 {(!account.user||reset||reauth)?<>
  {!reset&&<div className="account-tabs"><button disabled={busy} aria-pressed={mode==='login'} onClick={()=>{setMode('login');setError('');setNotice('');}}>Connexion</button><button disabled={busy||!signupAllowed} aria-pressed={mode==='signup'} onClick={()=>{setMode('signup');setError('');setNotice('');}}>Créer un compte</button></div>}
  {!reset&&!settings&&!settingsError&&<p role="status">Vérification du service d’inscription…</p>}
  {!reset&&(settingsError||settings?.disableSignup||settings?.autoconfirm)&&<p role="status">La création de compte avec confirmation e-mail est temporairement indisponible. Vous pouvez réessayer plus tard ou vous connecter à un compte existant.</p>}
  <form onSubmit={submit}>
   ...
   <button className="btn-primary" disabled={busy||(!reset&&mode==='signup'&&!signupAllowed)}>{busy?'Un instant…':reset?'Enregistrer le mot de passe':mode==='signup'?'Créer mon compte':mode==='forgot'?'Recevoir un lien':'Me connecter'}</button>
  </form>
  {!reset&&<button disabled={busy} className="btn-ghost" onClick={()=>{setMode(mode==='forgot'?'login':'forgot');setError('');setNotice('');}}>{mode==='forgot'?'Retour à la connexion':'Mot de passe oublié ?'}</button>}
