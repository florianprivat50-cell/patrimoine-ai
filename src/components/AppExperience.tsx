import { AccountStatus } from './AccountAccess';
...
export default function AppExperience(){
 ...
 return <>
  <header className="app-toolbar"><a href="#main-content" className="skip-link">Aller au contenu</a><div><span className="toolbar-brand">Patrimoine <b>AI</b></span><span className="toolbar-caption">Votre espace d’investissement</span></div><div className="toolbar-actions"><span className="connection-state"><i className={online?'':'offline'}/>{online?'Espace personnel':'Hors connexion'}</span>{!standalone&&<button className="install-button" disabled={installing} onClick={install}>{installing?'Installation…':'Installer l’application'}</button>}{standalone&&<span className="standalone-label">Application</span>}</div></header><AccountStatus/>
  ...
  <dialog ref={dialog} className="install-dialog" aria-labelledby="install-title" onClick={e=>{if(e.target===dialog.current)dialog.current.close()}}><button className="dialog-close" onClick={()=>dialog.current?.close()} aria-label="Fermer les instructions">Fermer</button><img src="/icon-192.png" alt="" width="64" height="64"/><span className="eyebrow">VOTRE ESPACE, À PORTÉE DE MAIN</span><h2 id="install-title">Patrimoine AI.<br/>Comme une application.</h2><p>Une fenêtre dédiée, un accès depuis votre écran d’accueil et vos dossiers accessibles hors connexion après une première visite.</p><ol>{ios?<><li>Ouvrez le site dans Safari.</li><li>Ouvrez le menu Partager.</li><li>Choisissez « Sur l’écran d’accueil », puis « Ajouter ».</li></>:<><li>Ouvrez le menu de votre navigateur.</li><li>Choisissez « Installer l’application » ou « Ajouter à l’écran d’accueil » si l’option est proposée.</li><li>Confirmez l’installation. Si l’option n’apparaît pas, essayez Chrome ou Edge.</li></>}</ol><p className="install-privacy">Connectez-vous au même compte pour retrouver vos dossiers sur ordinateur et téléphone. Le mode local reste limité à cet appareil.</p><button className="btn-primary" onClick={()=>dialog.current?.close()}>J’ai compris</button></dialog>
 </>;
}
