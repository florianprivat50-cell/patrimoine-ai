...
export function projectFeasibility(p:RealEstateProjectInputs, supplied?:Record<'prudent'|'realiste'|'optimiste',ProjectResults>) {
  ...
  const e=p.evidence;
  const fresh=e && Date.now()-Date.parse(e.retrievedAt)<90*86400000;
  ...
  const valid=[p.agencyFees,p.notaryFeesPct,p.works,p.furniture,p.bankFees,p.downPayment,p.ratePct,p.insurancePctYearly,p.monthlyCharges,p.propertyTaxYearly,p.ownerInsuranceYearly,p.managementPct,p.maintenancePct,p.taxRatePct].every(v=>Number.isFinite(v)&&v>=0)&&p.taxRatePct<100&&p.managementPct+p.maintenancePct<100&&Number.isFinite(p.price)&&p.price>0&&Number.isFinite(p.monthlyRent)&&p.monthlyRent>0&&p.durationYears>0&&p.vacancyPct>=0&&p.vacancyPct<100;
  const score=computeFeasibilityScore({askingPrice:p.price,totalCost:results.realiste.totalCost,subjectMonthlyRent:results.realiste.grossMonthlyRent,targetNetYieldPct:p.targetNetYieldPct,breakEvenRent:results.realiste.breakEvenRent,
    ...
    market:{...e?.market,subjectPricePerSqm:p.surface? p.price/p.surface:undefined,dataConfidence:fresh?e?.market.dataConfidence:'unknown'},
    propertyRisk:{...e?.risks,dataConfidence:fresh?e?.risks.dataConfidence:'unknown'},
