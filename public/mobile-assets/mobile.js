...
function renderFeasibility(m){
 let box=$('#analysis-feasibility');if(!box){box=document.createElement('section');box.id='analysis-feasibility';box.className='feasibility-card';box.setAttribute('aria-label','Note de faisabilité');$('#result-basis').after(box);}
 const f=m.feasibility;
 if(!f.calculable){box.innerHTML='<h3>Note de faisabilité</h3><p>Non calculable : complétez les données financières du projet.</p>';return;}
 box.innerHTML='<div class="feasibility-heading"><div><h3>Note de faisabilité</h3><p>'+esc(f.verdict)+' · '+(f.hardCaps.length?'note provisoire':'selon vos hypothèses')+'</p></div><strong class="feasibility-number">'+esc(f.score)+'<small>/100</small></strong></div><p>Calcul avant impôt, sur le scénario sélectionné. Cette note n’est ni une probabilité de réussite ni un accord bancaire.</p><details><summary>Comprendre la note</summary><div class="feasibility-components">'+f.components.map(c=>'<div><div class="feasibility-row"><span>'+esc(c.label)+'</span><strong>'+esc(dec(c.score*c.weight/100))+' / '+esc(c.weight)+'</strong></div><p>'+esc(c.explanation)+'</p></div>').join('')+'</div><p>Les critères non documentés peuvent recevoir une valeur neutre de 50/100 : cela ne signifie pas qu’ils ont été vérifiés. Des plafonds et pénalités peuvent réduire la somme des points.</p>'+(f.hardCaps.length?'<h4>Ce qui limite la note</h4><ul>'+f.hardCaps.map(x=>'<li>'+esc(x)+'</li>').join('')+'</ul>':'')+(f.redFlags.length?'<h4>Points de vigilance</h4><ul>'+f.redFlags.map(x=>'<li>'+esc(x)+'</li>').join('')+'</ul>':'')+'</details>';
}
function renderRobustness(m){
 renderFeasibility(m);
 let box=$('#analysis-robustness');if(!box){box=document.createElement('section');box.id='analysis-robustness';box.className='local-box';box.setAttribute('aria-label','Résistance du projet');$('#ledger').after(box);}
